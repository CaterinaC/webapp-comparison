...
@st.cache
def get_data():
    dat = pd.read_csv(Path(__file__).parent / "NEED_data_explorer_2021.csv",
                      encoding="latin-1",
                      na_values="n/a")
    return dat
...
@st.cache
def create_forecasts(ts):
    forecast_collector = []
    for build_age in ts["Attribute 2"].unique().tolist():
...
model_output = create_forecasts(dt_chunk)

# st.write(model_output)

fig = px.line(model_output,
              x="ds",
              y="y",
